...
Механизм временного закрепления товара за документом до его фактического списания. Бронирование увеличивает поле `reserved` в таблице остатков, тем самым уменьшая "доступный для обещания" остаток.

### Ghost Reservation (Призрачная бронь)

Ситуация, когда товар заблокирован в черновике, который никогда не будет проведен (например, из-за забывчивости пользователя). В системе Trade3 такие брони автоматически аннулируются специальным планировщиком задач (Cleanup Task).
