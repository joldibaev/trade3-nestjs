...
**Системный аудит изменений истории.**
Специальный тип документа, который фиксирует результаты работы механизма _Sequence Reprocessing_.

- В нем сохраняются старые и новые значения остатков и себестоимости, а также количество затронутых записей в истории (`StockLedger`). Это делает процесс пересчета прозрачным и проверяемым.
