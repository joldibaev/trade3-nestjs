...
2.  **COMPLETED -> DRAFT / CANCELLED**:
    ...
    - **Механизм**: Не удаляет старые записи, а добавляет новые "компенсирующие" записи в `PriceLedger` (с обратными значениями), чтобы сохранить полную историю изменений.
    - Пересчитывает текущую цену (`Price`) на основе последней записи в истории.
