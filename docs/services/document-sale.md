...
### `complete(id: string)`
Переводит документ продажи из `DRAFT` в `COMPLETED`. 
*   **Важно**: В этот момент система считывает текущую себестоимость из `Stock` и фиксирует её в позициях документа (`costPrice`). Затем происходит списание остатков.
