...
4. **Цены**: Если переданы `newPrices`, синхронизирует их со связанным документом `DocumentRevaluation`.
...
3. **Цены**: Если удаленные товары имели `newPrices`, они удаляются из связанного `DocumentRevaluation`.
...
1.  **Транзакция (`isolationLevel: 'ReadCommitted'`)**:
    - Весь процесс обернут в транзакцию. Для защиты от гонки данных при пересчете остатков используются **Advisory Locks** (через `inventoryService.lockProduct`).
...
5.  **Сценарий: Отмена проведения (COMPLETED -> DRAFT / CANCELLED)**:
    ...
    - **Откат движения**: Вызывает `applyInventoryMovements` с **отрицательным количеством** и причиной `REVERSAL`, чтобы:
      - Уменьшить остаток.
      - Скорректировать WAP (создается сторнирующая запись).
...
7.  **Пересчет истории (Sequence Reprocessing)**:
    ...
    - Если документ занесен задним числом или его статус изменен с `COMPLETED`, запускается процесс пересчета.
    ...
    - При обнаружении расхождений система автоматически создает корректирующие записи (`CORRECTION`) в `StockLedger`.
...
- **Изоляция**: Используется уровень `ReadCommitted` совместно с **Advisory Locks** для предотвращения конфликтов записи (Lost Updates) без накладных расходов `Serializable`.
