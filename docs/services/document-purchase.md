...
- **Code**: Генерируется автоматически с префиксом `P-` (например, `P-1`).
...
3. **Автоматическое изменение цен**:
    - Если в DTO для позиций передан массив `newPrices`, система автоматически создает связанный документ `DocumentPriceChange` в статусе **DRAFT**.
    - В этом документе будут зафиксированы новые цены на товары для указанных типов цен (`priceTypeId`).
    - Один документ изменения цен может содержать обновления для множества товаров и типов цен, если они были указаны в одной закупке.
    - Ссылка на закупку сохраняется в поле `documentPurchaseId` документа изменения цен.
4.  **Авто-планирование**: Если дата больше текущей, статус `COMPLETED` заменяется на `SCHEDULED`. Движения склада не создаются.
...
- **Логика**:
  ...
  - Создает новые `items` из DTO, включая `newPrices` для автоматического изменения цен.
  - Обновляет метаданные документа (дата, поставщик и т.д.).
  - Автоматически пересчитывает `total`.
  - **Автоматическое изменение цен**: Как и при создании, если переданы `newPrices`, обновляет или создает связанный `DocumentPriceChange`.
