...
4.  **Сценарий: Проведение (DRAFT -> COMPLETED)**:
    ...
    - **Обновление цен**: Вызывает `applyDelayedProductPriceUpdates`, чтобы применить новые розничные цены.
      - Использует логику **"Срез последних" (Slice Last)**: Текущая цена (`Price`) обновляется только если дата этого документа *новее* (или равна), чем дата текущей активной цены. Это предотвращает перезапись актуальных цен при редактировании документов задним числом.
      - Всегда создает запись в `PriceHistory` с датой документа.
