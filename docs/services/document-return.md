...
*   **Code**: Генерируется автоматически с префиксом `R-` (например, `R-1`).

1.  **Транзакция**:
    *   Создает `DocumentReturn`.
    *   Если статус сразу `COMPLETED`, выполняет оприходование (`InventoryService.getFallbackWapMap` + `stock.upsert`).
...
Удаление возврата.
*   **Ограничение**: Статус **DRAFT**.
*   **Логика**: Удаляет документ и позиции.
