...
2.  **Влияние на WAP**: Возврат обрабатывается как поступление, но (в зависимости от бизнес-настройки) обычно не влияет на WAP (используется текущая себестоимость), чтобы предотвратить манипуляции стоимостью склада через фиктивные возвраты.
