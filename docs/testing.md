...
В E2E тестах логирование ошибок отключено через `jest-e2e-setup.ts` для предотвращения "шума" в выводе тестов. Ошибки валидации и конкурентности (например, "Insufficient stock", "Transaction write conflict") являются ожидаемым поведением и корректно обрабатываются тестами.
