...
71: *   **`document-purchase.e2e-spec.ts`**:
72:     *   **Отложенное** обновление цен продажи (`newPrices`): цены сохраняются в документе и применяются только при проведении.
73:     *   Пересчет себестоимости (WAP).
