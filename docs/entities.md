...
В отличие от других документов, управление позициями в `DocumentPurchase` осуществляется гранулярно:
- Документ создается пустым (только заголовок).
- Товары добавляются, изменяются и удаляются через отдельные операции, что позволяет системе автоматически пересчитывать `total` документа и синхронизировать изменения цен (`DocumentPriceChange`) в реальном времени.

**Синхронизация с изменением цен**:
`DocumentPurchase` является мастер-документом для связанного `DocumentPriceChange`. 
- При добавлении/редактировании позиции закупки с указанием "Новых цен" (`newPrices`), автоматически создается или обновляется связанный документ изменения цен.
