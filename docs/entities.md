...
Архив всех изменений цен. Заполняется автоматически при проведении закупок или ручном изменении.
- `id`: UUID (v7)
- `value`: Историческое значение цены (Decimal(12, 2)).
- `productId`: Ссылка на товар.
- `priceTypeId`: Ссылка на тип цены.
- `documentPurchaseId`: Ссылка на документ закупки (если цена пришла оттуда).
- `date`: Дата "действия" цены (обычно совпадает с датой документа). Используется для определения текущей актуальной цены по методу "Срез последних".
- `createdAt`: Когда эта цена была физически добавлена в базу.
